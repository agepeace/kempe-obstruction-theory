...
在此基础上，主证明对极小5-色平面图施行**对称拼接法**：

将含唯一 GLFHO 的图 G(n) 与其副本对称拼接（挖空中心顶点，错位贴合边界顶点），通过极小化着色选择（定理一唯一性保证）使原侧 u 邻域不再是 GLFHO（子图引理：G ⊆ H 保证 G 中限制着色下 u 也非 GLFHO）。此时，**命题 3.1** 严格保证：若 u 非 GLFHO，则 v₁ 与 v₃ 必定在不同的 (1,3)-Kempe 链（反证：若同链则 Jordan 曲线 + 颜色分隔引理强制 u 成为 GLFHO，矛盾），从而通过 Kempe 链交换操作可为 u 完成4-着色，得到 G 的正常4-着色，与 χ(G)=5 矛盾。

证明的逻辑链为：定理一（唯一性）→ 对称拼接（子图引理 + 极小化着色）→ 命题 3.1（非 GLFHO ↔ v₁v₃ 异链，交换完成）→ 矛盾。各步均严格论证，无待形式化的悬留假设。
...
以上两条定理共同支撑了四色定理的证明路径。其核心机制为**对称拼接法**：

1. **GLFHO转移**：对极小5-色图 G 中唯一的 GLFHO（中心为5-度顶点 u），选取另一个与 u 不相邻的5-度顶点 v，通过定理二的对称拼接构造（挖空v和v'，错位贴合边界顶点），使 GLFHO 从 u 转移到 v。
