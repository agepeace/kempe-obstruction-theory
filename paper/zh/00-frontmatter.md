...
将含唯一 GLFHO 的图 G(n) 与其副本对称拼接，通过极小化着色选择（定理一唯一性保证）使原侧 u 邻域不再是 GLFHO（子图引理：G ⊆ H 保证 G 中限制着色下 u 也非 GLFHO）。此时，**命题 3.1** 严格保证：若 u 非 GLFHO，则 v₁ 与 v₃ 必定在不同的 (1,3)-Kempe 链（反证：若同链则 Jordan 曲线 + 颜色分隔引理强制 u 成为 GLFHO，矛盾），从而通过 Kempe 链交换操作可为 u 完成4-着色，得到 G 的正常4-着色，与 χ(G)=5 矛盾。
